...
def write_reference_folder(out_dir, pmg_structure, calc_text, species_meta, pp_path,
                            force_spin=False, force_dipole=False):
    ...
    reference -- and the slab dipole correction (DIPOLE_CORRECTION_BLOCK)
    ...
    an all-around vacuum box. Returns the FdfStructure written, in case a
    caller needs the exact same geometry object again without re-reading
    the file.
    """
...
def write_site_plot(pmg_structure, out_path, show=False):
    ...
    live on a real user structure). cluster_candidate_coords() re-wraps
    them all into one consistent periodic image first. Marker style
    (color/marker/markersize/mew/zorder) copied exactly from plot_slab's
    own adsorption_sites=True code path, so the plot looks identical apart
    from this fix.
    ...
    ads_sites = cluster_candidate_coords(ads_sites, pmg_structure.lattice.matrix)
    if ads_sites:
        symm_op = get_rot(pmg_structure)
        xy = [symm_op.operate(site)[:2].tolist() for site in ads_sites]
