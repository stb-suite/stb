# 1.3 — K-Path Generator (`stb-kpath`)
...
`stb-kgrid` (example `1.2`) builds a Monkhorst-Pack **grid** — k-points
...
arrangement is taken into account. `example_1.3.sh` shows exactly this on
...
genuinely periodic direction (see example `1.2`'s own theory section). An
...
`1.3` asks for the structure file, then — the same "advanced settings"
...
`example_1.3.sh` proves both paths agree.
...
- `example_1.3.sh` — the guided walkthrough (**not** an automated test —
  see `test/1-inputs/3-k-path/test.sh` for that). Pauses between sections
...
Same idea as example `1.2`: a structure with a 12 Ang gap along `c` sits
