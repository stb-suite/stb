...
# --- Setup ---
# Smoke test for stb-hubbarduAnalysis (Hubbard U Linear Response, Analysis, item 4.7.2)
...
# known ground truth: U = 1/chi0 - 1/chi = 1/0.20 - 1/0.30 = 1.666667 eV.
echo "--- Starting tester for STB-HubbarduAnalysis (item 4.7.2) ---"
rm -rf "$TEST_DIR"
mkdir -p "$TEST_DIR"
...
data = {'reference': 5.0, 'scf_alpha_0.1000': 5.2, 'scf_alpha_-0.1000': 5.15,
        'frozen_alpha_0.1000': 5.02, 'frozen_alpha_-0.1000': 4.99}
...
python3 -c "
import json
manifest = {'species': 'Fe', 'n': 3, 'l': 2, 'label': 'siesta', 'runs': {
    'reference': {'kind': 'reference', 'alpha': 0.0},
    'scf_alpha_0.1000': {'kind': 'scf', 'alpha': 0.1},
}}
...
echo "Testing: navigate 4.7.2 -> defaults -> quit"
rm -f Mn_LDAU.fdf
