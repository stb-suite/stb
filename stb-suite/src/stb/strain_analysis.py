...
def write_strain_gplot(curve_filename, gplot_filename, direction, results, unit):
    ...
    # File references INSIDE the .gplot script must be bare basenames, not
    # curve_filename/gplot_filename's own (possibly --output-dir-prefixed)
    # path: gnuplot resolves them relative to ITS OWN cwd at execution
    # time, and the natural usage is "cd <output-dir> && gnuplot
    # <script>.gplot" -- the script already lives next to its data file, so
    # re-prepending output-dir would look for output-dir/output-dir/....
    curve_basename = os.path.basename(curve_filename)
    pdf_basename = os.path.basename(gplot_filename).rsplit(".", 1)[0] + ".pdf"
    lines = [
        '# --- STB Plot Configuration ---\n',
        '# Generated by stb-strainAnalysis\n',
        'set terminal pdfcairo enhanced color font "Arial,14" size 7,5\n',
        f'set output "{pdf_basename}"\n\n',
        ...
        (f'plot "{curve_basename}" index 0 using 2:3 with linespoints pt 7 ps 1.2 lc rgb "#2255cc" '
         f'title "{qty}-strain data", \\\n'
         f'     (x>={fit_lo:.8f} && x<={fit_hi:.8f}) ? f(x) : 1/0 with lines lc rgb "#2255cc" '
         'dt 2 title "Initial slope fit", \\\n'
         f'     "{curve_basename}" index 1 using 2:3 with points pt 9 ps 2 lc rgb "#cc2222" '
         'title "Peak"\n')
    ]
...
def write_compare_gplot(data_filename, gplot_filename, per_direction, unit):
    ...
    # File references INSIDE the .gplot script must be bare basenames --
    # see write_strain_gplot's own comment on why (gnuplot resolves them
    # relative to its own cwd when run, not the writer's).
    ...
    for i, direction in enumerate(directions):
        ...
        plot_parts.append(
            f'"{data_basename}" index {idx0} using 2:3 with linespoints pt 7 ps 1.2 lc rgb "{color}" '
            f'title "{direction.upper()}"')
        plot_parts.append(
            f'(x>={fit_lo:.8f} && x<={fit_hi:.8f}) ? f_{i}(x) : 1/0 with lines lc rgb "{color}" '
            'dt 2 notitle')
        plot_parts.append(
            f'"{data_basename}" index {idx1} using 2:3 with points pt 9 ps 2 lc rgb "{color}" notitle')
    lines.append('plot ' + ', \\\n     '.join(plot_parts) + '\n')
...
def main():
    ...
    if args.save_gnuplot:
        if len(results_by_dir) == 1:
            ...
            render_gplot = compare_gplot
        print_dual(color_text(
            f"[OK] Curve data + gnuplot script written: {len(written_files)} file(s).", 'green'), f_out)
        for path in written_files:
            print_dual(f"  - {path}", f_out)
        print_dual(f"  Render with: cd {args.output_dir} && gnuplot "
                   f"{os.path.basename(render_gplot)}  (the .gplot script references its "
                   "data file/output by bare filename, so it must be run from inside this "
                   "same directory).", f_out)
    else:
