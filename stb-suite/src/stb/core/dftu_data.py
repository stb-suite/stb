...
# Fields every run_manifest.json written by stb-hubbardu must have --
# checked by load_manifest() so a malformed/hand-edited/pre-3-stage-split
# manifest fails with a clean message instead of a raw KeyError the first
# time some later field access happens to hit the missing key.
REQUIRED_MANIFEST_FIELDS = ("species", "n", "l", "j", "label", "runs")
...
def load_manifest(manifest_path):
    """Loads and validates a run_manifest.json, raising ValueError with a
    clean, user-facing message (never a raw KeyError surfacing later at some
    unrelated line) if it's malformed or missing a required top-level field
    -- e.g. a manifest left over from an older/incompatible version of this
    workflow, or hand-edited by a curious user.
    """
    ...
    if missing:
        raise ValueError(
            ...
            "generated by the current stb-hubbardu workflow (maybe an older/incompatible version, "
