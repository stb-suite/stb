# 1.4 — DFT+U / Hubbard Block Generator (`stb-dftu`)
...
`1.4` itself offers two sub-modes: **[1]** read species off a structure
...
prompts. `example_1.4.sh` proves the CLI and the menu agree.
...
- `example_1.4.sh` — the guided walkthrough (**not** an automated test —
  see `test/1-inputs/4-dftu/test.sh` for that). Pauses between sections so
