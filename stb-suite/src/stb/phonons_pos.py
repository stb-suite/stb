...
# 1 THz*Ang = (1e-10 m)*(1e12 /s) = 1e2 m/s -- phonopy's documented native
# group-velocity unit (frequency in THz, q in the reciprocal-lattice
# convention it already uses internally, 2*pi/Ang), no separate conversion
# needed beyond this factor.
THZ_ANG_TO_KM_PER_S = 0.1


def write_group_velocity_plot(plot_dir, bs, path_connections, f_out):
    """group_velocity.dat (distance + |group velocity| per band, same
    segment/column layout as bands.dat) + .gplot. Reuses bs.group_velocities
    already computed by run_band_structure(with_group_velocities=True) --
    no recomputation. Magnitudes in phonopy's native THz*Ang; see
    THZ_ANG_TO_KM_PER_S for the km/s conversion used in the report text.
    """
    os.makedirs(plot_dir, exist_ok=True)
    gv_mag = [np.linalg.norm(seg, axis=2) for seg in bs.group_velocities]
    n_bands = gv_mag[0].shape[1]

    dat_path = os.path.join(plot_dir, "group_velocity.dat")
    with open(dat_path, "w") as f:
        f.write("# stb-phononsPos -- phonon group velocity magnitude |v_g| = |d(omega)/dq|\n")
        f.write(f"# columns: 1=distance(1/Ang) 2..{n_bands + 1}=|v_g| per band (THz*Ang, "
                f"x{THZ_ANG_TO_KM_PER_S * 1000:.0f} for m/s)\n")
        for seg_idx, (dist, mags) in enumerate(zip(bs.distances, gv_mag)):
            if seg_idx > 0 and not path_connections[seg_idx - 1]:
                f.write("\n\n")
            for d, row in zip(dist, mags):
                f.write(f"{d:12.6f} " + " ".join(f"{v:12.6f}" for v in row) + "\n")

    gplot_path = os.path.join(plot_dir, "group_velocity.gplot")
    with open(gplot_path, "w") as f:
        f.writelines([
            '# --- STB Plot Configuration ---\n',
            '# Generated by stb-phononsPos\n',
            'set terminal pdfcairo enhanced color font "Arial,14" size 8,6\n',
            'set output "group_velocity.pdf"\n\n',
            'set title "Phonon Group Velocity Magnitude"\n',
            'set xlabel "Distance (1/Ang)"\n',
            'set ylabel "|v_g| (THz*Ang)"\n',
            'set grid\n',
            'unset key\n',
            f'plot for [i=2:{n_bands + 1}] "group_velocity.dat" using 1:i with lines '
            f'lc rgb "{PLOT_COLORS[1]}"\n',
        ])

    print_dual(f"{color_text('[Saved]', 'cyan')} {dat_path}, {gplot_path} "
                f"(cd {plot_dir} && gnuplot {os.path.basename(gplot_path)})", f_out)
    return [("group_velocity", dat_path, gplot_path)]
