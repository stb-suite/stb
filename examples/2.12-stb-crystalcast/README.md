...
| **`stb-symmetry`** / **`stb-unitcell`** | Analyze an *existing* structure's own symmetry (operations, equivalent atoms, primitive/conventional cell) | This is what `--analyze` used to do here, before that mode was removed. `stb-crystalcast` **generates or transforms**; it doesn't have an inspection-only mode of its own anymore. |
