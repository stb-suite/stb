...
# --- Setup ---
# Smoke test for stb-kpath (K-Path Generator, item 1.4)
...
# --- 1. Preparation ---
echo "--- Starting tester for STB-KPath (item 1.4) ---"
rm -rf "$TEST_DIR"
mkdir -p "$TEST_DIR"
...
echo "Testing: navigate 1.4 -> invalid file then valid -> skip advanced settings (N) -> quit"
rm -f kpath_bs.fdf
...
echo "Testing: navigate 1.4 -> valid file -> open advanced settings (y) -> custom vacuum-gap/eps/symprec/angle -> quit"
