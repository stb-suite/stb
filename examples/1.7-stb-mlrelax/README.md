# 1.7 — ML Pre-Relaxation (`stb-mlrelax`)
...
`example_1.7.sh` proves the CLI and the menu agree.
...
- `example_1.7.sh` — the guided walkthrough (**not** an automated test —
  see `test/1-inputs/7-mlrelax/test.sh` for that). Pauses between
  sections so you can read before moving on; safe to re-run.
- `output/` — created by `example_1.7.sh` when you run it (git-ignored, not
