...
#     Now with Energy vs Stress Validation      #
#################################################
...
| '--------------' || '--------------' || '--------------' |
 ...
 """, 'cyan')
    ...
    description = [
        ...
        "Stress-Strain vs Energy-Strain Validation",
...
def validate_stress_vs_energy(strain, energy, volume_list, c_stress, label, conv_factor, f_out):
    """
    Calculates stiffness from Energy vs Strain and compares with Stress vs Strain.
    Used to detect Pulay Stress issues.
    """
    if len(strain) < 3 or len(energy) < 3:
        return # Not enough points for quadratic fit

    # 1. Polynomial Fit (E = a*x^2 + b*x + c)
    # coeffs[0] = a, coeffs[1] = b, coeffs[2] = c
    coeffs = np.polyfit(strain, energy, 2)
    a = coeffs[0]
    
    # 2. Average Volume
    avg_vol = np.mean(volume_list)
    
    # 3. Calculate C from Energy Curvature
    # C = (1/V) * (d2E/de2) = (1/V) * 2*a
    # Result in eV/Ang^3
    c_energy_raw = (2 * a) / avg_vol
    
    # 4. Convert to final units (GPa or N/m)
    c_energy = c_energy_raw * conv_factor
    
    # 5. Compare
    diff = abs(c_energy - c_stress)
    mean_val = (abs(c_energy) + abs(c_stress)) / 2.0
    percent_diff = (diff / mean_val) * 100 if mean_val > 1e-5 else 0.0
    
    # Visuals
    color = 'green'
    if percent_diff > 5.0: color = 'yellow'
    if percent_diff > 10.0: color = 'red'
    
    print_dual(f"  {label:<4} | Stress: {c_stress:8.2f} | Energy: {c_energy:8.2f} | Diff: {color_text(f'{percent_diff:5.1f}%', color)}", f_out)
    
    if percent_diff > 10.0:
        print_dual(f"        {color_text('-> WARNING: Large discrepancy! Check MeshCutoff/Pulay Stress.', 'red')}", f_out)
...
def main():
    ...
    for folder in folders:
        ...
        # --- MODIFICATION: Store Energy and Volume too ---
        msg = f"   -> {folder:<25} : "
        if S is not None:
            if direction not in data: 
                data[direction] = {'eps': [], 'stress': [], 'energy': [], 'volume': []}
            
            data[direction]['eps'].append(strain)
            data[direction]['stress'].append(S)
            if E is not None: data[direction]['energy'].append(E)
            if V is not None: data[direction]['volume'].append(V)
    ...
    # --- C_ij Calculation (Stress Method) ---
    C = np.zeros((6, 6))
    ...
    # --- VALIDATION (Energy vs Stress) ---
    print_dual(f"\n{color_text('[3] METHOD VALIDATION (PULAY STRESS CHECK)', 'magenta')}", f_out)
    print_dual("Comparing Stress-Strain (C_ij) vs Energy-Strain (d2E/de2)", f_out)
    print_dual("-" * 60, f_out)
    
    # Check C11 (xx)
    if 'xx' in data and len(data['xx']['energy']) > 2:
        validate_stress_vs_energy(data['xx']['eps'], data['xx']['energy'], data['xx']['volume'], 
                                  C_sym[0,0], "C11", CONV_FACTOR, f_out)
    # Check C22 (yy)
    if 'yy' in data and len(data['yy']['energy']) > 2:
        validate_stress_vs_energy(data['yy']['eps'], data['yy']['energy'], data['yy']['volume'], 
                                  C_sym[1,1], "C22", CONV_FACTOR, f_out)
                                  
    print_dual("Note: Differences >10% indicate Pulay Stress issues (need higher MeshCutoff).", f_out)
