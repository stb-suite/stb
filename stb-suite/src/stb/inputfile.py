...
# exercises them: SIESTA, Monkhorst-Pack, and the PBE (XC.Functional GGA /
# XC.Authors PBE, hardcoded in every template above) are cited
# unconditionally whenever the k-grid was actually (re)computed; DFT-D3 and
# the Nose thermostat only when the generated input file actually turns them
# on (DFTD3 .true., or an AIMD template, whose MD.TypeOfRun is Nose).
# SIESTA + Monkhorst-Pack are shared with stb-kgrid via core/citations.py.
...
def build_bib_entries(chosen_mode, d3_enabled, pseudo_bank=None):
    ...
    entries = [citations.SIESTA, citations.SIESTA_RECENT, _BIB_PBE]
    if chosen_mode != 'aimd':
        # AIMD keeps the template's own k-grid unchanged (see [2] K-GRID
        # below) -- Monkhorst-Pack is only actually exercised otherwise.
        entries.append(citations.MONKHORST_PACK)
    if d3_enabled:
