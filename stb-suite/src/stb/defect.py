...
Select the site by raw atom index or by the position closest to a target.""",
    ...
    parser.add_argument("--filter-species", type=str, default=None,
                        help="Restrict --nearest's search to atoms of this element.")
    ...
    if args.type in ("vacancy", "substitution"):
        if args.index is None and args.nearest is None:
            parser.error("--type vacancy/substitution requires --index or --nearest.")
