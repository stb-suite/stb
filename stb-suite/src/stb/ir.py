...
def print_dual(text, file_handle=None):
    ...
    duplicated-per-tool helper as raman.py/phonons_create.py."""
    print(text)
    if file_handle:
...
phonon_disp/ tree).""",
    ...
    with open(report_path, "w") as f_out:
        ...
        if is_bulk:
            ...
            print_dual(color_text(
                "[NOTE] Non-bulk (0D/1D/2D) structure detected -- Stage 2 (stb-irModes) will "
                "write a +/-delta dipole-moment displacement pair per selected mode (SIESTA "
                "prints the total dipole automatically for a non-bulk system, no extra fdf flag "
                "needed).", 'cyan'), f_out)
