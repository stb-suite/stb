...
stb-symmetry --file structure.fdf --format fdf --symprec 1e-4 --no-intro > log_p1.txt 2>&1
check_exit_code $? 0
...
stb-symmetry --file siesta.STRUCT_OUT --format struct_out --symprec 1e-4 --no-intro > log_struct_out.txt 2>&1
check_exit_code $? 0
...
check_contains "# Detection tolerance: symprec=0.01, angle_tolerance=5" refined.fdf
...
echo -e "\n--- Testing --compare-to ---"
stb-symmetry --file structure.fdf --format fdf --compare-to siesta.STRUCT_OUT --compare-format struct_out --symprec 1e-4 --no-intro > log_compare_same.txt 2>&1
check_exit_code $? 0
...
stb-symmetry --file structure.fdf --format fdf --symprec 1e-4 --save-report --no-intro > log_saved.txt 2>&1
check_exit_code $? 0
...
stb-symmetry --file structure.fdf --format fdf --symprec 1e-4 --no-intro > log_stale.txt 2>&1
check_exit_code $? 0
...
echo -e "\n--- Testing --output-dir ---"
rm -rf out_dir
stb-symmetry --file structure.fdf --format fdf --output-dir out_dir --symprec 1e-4 --save-report --no-intro > log_outdir.txt 2>&1
check_exit_code $? 0
...
check_contains "Symmetry PRESERVED between the two structures: both P-1 (No. 2, 2 ops)." log_menu_compare.txt
