# 1.2 — K-Grid Generator (`stb-kgrid`)
...
command underneath. `example_1.2.sh` proves both paths agree.
...
- `example_1.2.sh` — the guided walkthrough (**not** an automated test — see
  `test/1-inputs/2-k-grid/test.sh` for that). Pauses between sections so you
  can read before moving on; safe to re-run.
- `output/` — created by `example_1.2.sh` when you run it (git-ignored, not
